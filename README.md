...
我只在你的 Windows 本机工作，服务默认监听 `127.0.0.1`。虽然很想帮忙，但不会绕过付费墙，也不支持 Sci-Hub 或其他未授权全文源——守规矩的小猫才能安心做研究。
...
`runtime/config.toml`；联系邮箱和 EndNote 路径保存在本地数据库，机构凭据保存在 Windows 凭据管理器：
