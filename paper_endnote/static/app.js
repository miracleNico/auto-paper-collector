...
async function loadSystem() {
  ...
  $("#library-options").innerHTML = state.system.libraries.map(path => `<option value="${esc(path)}"></option>`).join("");
}
...
async function renderBatch() {
  ...
  const managerLabel = batch.reference_manager === "zotero" ? "Zotero" : "EndNote";
  $("#task-detail").innerHTML = `
    <div class="task-head"><div><h2>${esc(batch.name)}</h2><p class="muted">${esc(batch.target_library)}</p></div>
      <div class="task-actions">
        ${batch.status === "running" ? `<button data-action="pause">暂停</button>` : `<button data-action="resume">继续匹配/获取</button>`}
        <button class="primary" data-action="commit">提交到 ${managerLabel}</button>
    ...
    <div class="summary-grid">
      ...
      <div class="summary-card"><b>${counts.endnote}</b><span>${managerLabel} 已核验</span></div>
      <div class="summary-card"><b>${counts.needs}</b><span>需要处理</span></div>
    </div>
    ${batch.error ? `<p class="error-text">${esc(batch.error)}</p>` : ""}
    <div class="table-wrap"><table><thead><tr><th>#</th><th>论文</th><th>题录</th><th>PDF</th><th>${managerLabel}</th><th>状态/操作</th></tr></thead>
...
async function batchAction(action) {
  try { await api(`/api/batches/${state.selectedBatch}/${action}`, {method:"POST",body:"{}"}); toast(action === "commit" ? "文献库提交已启动" : "状态已更新"); await renderBatch(); }
  catch (error) { toast(error.message); }
}
...
$("#batch-form").onsubmit = async event => {
  ...
  try {
    const result = await api("/api/batches", {method:"POST",body:JSON.stringify({name:$("#batch-name").value,items_text:$("#items-text").value,target_library:$("#target-library").value,library_mode:$("#library-mode").value,reference_manager:$("#reference-manager").value,start_immediately:true})});
    state.selectedBatch = result.id; showTab("tasks"); toast("批次已创建");
  } catch (error) { toast(error.message); }
};
$("#settings-form").onsubmit = async event => { event.preventDefault(); try { await api("/api/settings", {method:"POST",body:JSON.stringify({crossref_email:$("#crossref-email").value,unpaywall_email:$("#unpaywall-email").value})}); toast("设置已保存"); } catch (error) { toast(error.message); } };
...
$("#reference-manager").onchange = () => {
  const zotero = $("#reference-manager").value === "zotero";
  $("#target-label").textContent = zotero ? "Zotero collection 名称" : "EndNote 库路径";
  $("#target-help").textContent = zotero ? "Zotero 中将新建或复用这个 collection。" : "新建库时填写尚不存在的 .enl 文件；已有库首次提交前创建备份。";
  $("#target-library").placeholder = zotero ? "DTN" : "C:\\Users\\...\\Research.enl";
  if (zotero && !$("#target-library").value) $("#target-library").value = "DTN";
};
